...
export async function runHeavy(rawInput: RunHeavyOptions): Promise<HeavyFinal> {
  const { adapterFactory: injectedAdapterFactory } = rawInput;
  ...
  const adapterFactory = injectedAdapterFactory ?? ((agent: AgentRequest) => {
